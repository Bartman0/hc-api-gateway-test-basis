...
class TestClass:
    ...
    def test_amsterdam_landelijk_validation(
        ...
        if not is_amsterdam_authorised(token):
            assert not has_amsterdam_query_parameter(request_result)

    def test_request_filters(
            self, hc_ams_request, hc_api_request, token, status_code
    ):
        request_result = transform_request_filters(hc_ams_request, token)
        request_result = unify_fields(request_result)
        hc_api_request = unify_fields(hc_api_request)
